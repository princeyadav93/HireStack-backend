...
Last updated: 8 August 2026 · Branch: `dev`
...
**57 endpoints across 9 modules**, plus a health check. All wired and reachable.

| Area                  | What works                                                                                                            |
| --------------------- | --------------------------------------------------------------------------------------------------------------------- |
| **Auth**              | Register, login, logout, refresh-token rotation. JWT in httpOnly cookies. Logout revokes every existing token.          |
| **Account recovery**  | Email verification and password reset over SMTP. Hashed single-use links; a reset signs every device out.               |
...
| **Tests**             | 144 tests — Vitest, supertest, in-memory MongoDB replica set. Unit + integration, run in CI on every push.              |
...
- **Token confusion is blocked.** Access and refresh tokens carry a `type` claim, and a
  `tokenVersion` counter makes logout genuinely revoke. The same idea covers email links:
  a reset token and a verification token are both 32 random bytes in one collection, and
  only the `type` on the lookup keeps either from being redeemed as the other.
- **A password reset actually ends the attacker's session.** It bumps `tokenVersion` and
  drops the stored refresh token, so every device is signed out — a reset that left live
  tokens alone would be theatre, since resetting is what you do when you fear compromise.
...
### 1. Two small production blockers
...
Deliberately last: step 2 adds an endpoint, so generating the spec before it means
regenerating it after.
...
| Item                                                                                             | Impact |
| ------------------------------------------------------------------------------------------------ | ------ |
| Nothing yet *requires* a verified email — the flag is set but no endpoint checks it                 | Medium |
| Test coverage skips companies and profiles                                                         | Medium |
...
```
✓  Tests                         ← 144 tests, running in CI
✓  Refresh-token hashing         ← SHA-256 + jti; rotation now actually revokes
✓  Password reset + email verify ← SMTP; hashed single-use links; reset ends all sessions
1. trust proxy + logging         ← ~1 hour, blocks deployment
2. Company logo upload           ← small, visible win
3. OpenAPI docs                  ← last, so the spec captures the finished surface
4. Frontend                      ← the API surface is ready for it
```
...
Only one deployment blocker is left, and it is an hour's work.
